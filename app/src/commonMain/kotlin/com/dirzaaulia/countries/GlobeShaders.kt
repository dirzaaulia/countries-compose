...
object GlobeShaders {
    ...
    const val FRAGMENT_SHADER = """
        ...
        void main() {
            ...
            vec3 V = vec3(0.0, 0.0, 1.0); // View vector pointing towards camera
            
            vec4 dayColor = texture2D(u_DayTexture, v_TexCoordinate);
            
            if (u_IsMoon > 0.5) {
                // -------------------------------------------------------------
                // Photorealistic Moon Renderer (NASA LRO Texture, no atmosphere)
                // -------------------------------------------------------------
                float NdotL = dot(N, L);
                ...
                // Night side: City lights + starlight/moonlight ambient glow on landmasses & islands
                float nightFactor = 1.0 - sunFactor;
                vec3 nightLandAmbient = dayColor.rgb * 0.16; // Continents, islands, and coastlines visible at night
                ...
                // Sunlit clouds with sunset fringe along terminator
